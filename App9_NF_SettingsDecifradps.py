...
def get_gymdbsettingsencnf(files_found, report_folder, seeker, wrap_text, time_offset):
    ...
    if len(all_rows) > 0:
        report = ArtifactHtmlReport('NF')
        report.start_artifact_report(report_folder, 'NF - Settings cifrados')
        report.add_script()
        ...
        logfunc('NF - Encrypted Settings data available in report under "NF - Settings Encrypted"')
    else:
        logfunc('No NF - Encrypted Settings data available')


__artifacts__ = {
    "NFSettingsEncrypted": (
        "App9_NF",
        ('*/data/data/pt.proinf.myhcgym.nf/databases/settings.db'),
        get_gymdbsettingsencnf)
}

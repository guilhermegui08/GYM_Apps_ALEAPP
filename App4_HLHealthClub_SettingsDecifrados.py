...
def get_gymdbsettingsencHLHealthClub(files_found, report_folder, seeker, wrap_text, time_offset):
    ...
    if len(all_rows) > 0:
        ...
        report.start_artifact_report(report_folder, 'HLHealthClub - Settings cifrados')
        report.add_script()
        ...
        logfunc('HLHealthClub - Encrypted Settings data available in report under "HLHealthClub - Settings Encrypted"')
    else:
        logfunc('No HLHealthClub - Encrypted Settings data available')


__artifacts__ = {
    "HLHealthClubSettingsEncrypted": (
        "App4_HLHealthClub",
        ('*/data/data/pt.proinf.myhcgym.hlhealthylife/databases/settings.db'),
        get_gymdbsettingsencHLHealthClub)
}

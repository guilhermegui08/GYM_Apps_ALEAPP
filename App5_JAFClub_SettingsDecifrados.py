...
def get_gymdbsettingsencJAFClub(files_found, report_folder, seeker, wrap_text, time_offset):
    ...
    if len(all_rows) > 0:
        ...
        report.start_artifact_report(report_folder, 'JAFClub - Settings cifrados')
        report.add_script()
        ...
        logfunc('JAFClub - Encrypted Settings data available in report under "JAFClub - Settings Encrypted"')
    else:
        logfunc('No JAFClub - Encrypted Settings data available')


__artifacts__ = {
    "JAFClubSettingsEncrypted": (
        "App5_JAFClub",
        ('*/data/data/pt.proinf.myhcgym.jafclub/databases/settings.db'),
        get_gymdbsettingsencJAFClub)
}

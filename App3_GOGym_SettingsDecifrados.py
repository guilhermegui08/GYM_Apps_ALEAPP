...
def get_gymdbsettingsencGOGym(files_found, report_folder, seeker, wrap_text, time_offset):
    ...
    if len(all_rows) > 0:
        ...
        report.start_artifact_report(report_folder, 'GOGym - Settings cifrados')
        report.add_script()
        ...
        logfunc('GOGym - Encrypted Settings data available in report under "GOGym - Settings Encrypted"')
    else:
        logfunc('No GOGym - Encrypted Settings data available')


__artifacts__ = {
    "GOGymSettingsEncrypted": (
        "App3_GOGym",
        ('*/data/data/pt.proinf.myhcgym.gogym/databases/settings.db'),
        get_gymdbsettingsencGOGym)
}

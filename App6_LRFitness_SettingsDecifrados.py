...
def get_gymdbsettingsencLRFitness(files_found, report_folder, seeker, wrap_text, time_offset):
    ...
    if len(all_rows) > 0:
        report = ArtifactHtmlReport('LRFitness')
        report.start_artifact_report(report_folder, 'LRFitness - Settings cifrados')
        report.add_script()
        ...
        logfunc('LRFitness - Encrypted Settings data available in report under "LRFitness - Settings Encrypted"')
    else:
        logfunc('No LRFitness - Encrypted Settings data available')


__artifacts__ = {
    "LRFitnessSettingsEncrypted": (
        "App6_LRFitness",
        ('*/data/data/pt.proinf.myhcgym.lrfitness/databases/settings.db'),
        get_gymdbsettingsencLRFitness)
}

...
def get_gymdbsettingsLRFitness(files_found, report_folder, seeker, wrap_text, time_offset):
    ...
    if len(all_rows) > 0:
        ...
        report.start_artifact_report(report_folder, 'LRFitness - Settings')
        report.add_script()
        data_headers = ('Key', 'Value', 'Hora')
        report.write_artifact_data_table(data_headers, all_rows, files_found[0])
        report.end_artifact_report()
        logfunc('LRFitness - Settings data available in report under "LRFitness - Settings"')
    else:
        logfunc('No LRFitness - Settings data not available')

__artifacts__ = {
    "LRFitnessSettings": (
        "App6_LRFitness",
        ('*/data/data/pt.proinf.myhcgym.lrfitness/databases/settings.db'),
        get_gymdbsettingsLRFitness)
}
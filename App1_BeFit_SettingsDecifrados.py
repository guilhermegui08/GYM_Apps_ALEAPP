...
def get_gymdbsettingsencbefit(files_found, report_folder, seeker, wrap_text, time_offset):
    ...
    if len(all_rows) > 0:
        ...
        report.start_artifact_report(report_folder, 'BeFit - Settings cifrados')
        report.add_script()
        ...
        logfunc('BeFit - Encrypted Settings data available in report under "BeFit - Settings Encrypted"')
    else:
        logfunc('No BeFit - Encrypted Settings data available')


__artifacts__ = {
    "BeFitSettingsEncrypted": (
        "App1_BeFit",
        ('*/data/data/pt.proinf.myhcgym.befit/databases/settings.db'),
        get_gymdbsettingsencbefit)
}

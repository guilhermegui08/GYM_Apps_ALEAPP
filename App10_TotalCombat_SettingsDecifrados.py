...
def get_gymdbsettingsenctotalcombat(files_found, report_folder, seeker, wrap_text, time_offset):
    ...
    if len(all_rows) > 0:
        ...
        report.start_artifact_report(report_folder, 'TotalCombat - Settings cifrados')
        report.add_script()
        ...
        logfunc('TotalCombat - Encrypted Settings data available in report under "TotalCombat - Settings Encrypted"')
    else:
        logfunc('No TotalCombat - Encrypted Settings data available')


__artifacts__ = {
    "TotalCombatSettingsEncrypted": (
        "App10_TotalCombat",
        ('*/data/data/pt.proinf.myhcgym.totalcombat/databases/settings.db'),
        get_gymdbsettingsenctotalcombat)
}

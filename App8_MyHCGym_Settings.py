...
def get_gymdbsettingsMyHCGym(files_found, report_folder, seeker, wrap_text, time_offset):
    ...
    if len(all_rows) > 0:
        ...
        report.start_artifact_report(report_folder, 'MyHCGym - Settings')
        report.add_script()
        data_headers = ('Key', 'Value', 'Hora')
        report.write_artifact_data_table(data_headers, all_rows, files_found[0])
        report.end_artifact_report()
        logfunc('MyHCGym - Settings data available in report under "MyHCGym - Settings"')
    else:
        logfunc('No MyHCGym - Settings data not available')

__artifacts__ = {
    "MyHCGymSettings": (
        "App8_MyHCGym",
        ('*/data/data/pt.proinf.myhcgym.myhcgym/databases/settings.db'),
        get_gymdbsettingsMyHCGym)
}